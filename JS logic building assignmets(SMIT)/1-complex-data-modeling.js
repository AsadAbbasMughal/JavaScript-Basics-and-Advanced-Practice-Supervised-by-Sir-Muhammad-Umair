...
// let a = products.map((p)=>{
//   return p.varations.reduce((acc , cur)=>{
...
// products.map((p)=>{
//   let total = p.reviews.reduce((acc, cur)=> acc + cur.rating, 0)
//   console.log(total /p.reviews.length);
// })
...
let a = products.map(({ id, title, reviews })=>{
  let a = reviews.filter((status)=> !status);
  return {id,title, a}
})
console.log(a);
